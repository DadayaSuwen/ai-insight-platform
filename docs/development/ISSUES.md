...
## Phase 9: 流式输出 + API Provider 修复（Bug #32–#35）

Phase 9 目标：后端 LLM 输出改造为真正的流式（token-by-token SSE），并修复 OpenAI 兼容 API provider 的流式调用问题。

共 4 个代码改动。

### Bug #32 — `LlmService.invoke()` 是同步等待，不是流式

🔴 阻塞（功能缺失）

**现象**：前端 SSE 流式端点 `/chat/stream` 虽然能连上，但后端是等 LLM 生成完整回答后，才一次性发送所有 SSE 事件。用户看到的是"等待 5 秒 → 突然全部出现"，而不是逐字打字效果。

**根因**：完整调用链路：
```
ChatController.stream()
  → ChatService.processMessageStream()
       → from(this.aiService.process(message))  ← ⚠️ 等待完整 promise
            → LlmService.invoke()
                 → chat.invoke(messages)        ← ⚠️ 同步等待完整响应
```

所有事件（token/sql/chart/analysis）都在 LLM 完全结束后才一次性发出去。

**修复**：
- `LlmService` 新增 `invokeStream()` 方法 — `async generator`，逐 token yield
- `AiService` 新增 `processStream()` + 4 个流式 handler（`handleChatStream`/`handleSqlStream`/`handleChartStream`/`handleAnalysisStream`）
- `ChatService.processMessageStream()` 改为 `new Observable()` 内部 async generator，逐事件即时 emit
- 75 个单元测试全部通过

**验证**：
```bash
$ curl "http://localhost:3000/chat/stream?message=hello"
event: token
data: {"content":"你","isFinal":false}
event: token
data: {"content":"好","isFinal":false}
event: token
data: {"content":"！","isFinal":false}
...
event: done
```
...
### Bug #33 — `ChatOpenAI.stream()` 返回 `Promise` 而非直接返回 `AsyncIterable`

🔴 阻塞（API Provider）

**现象**：切换到 `deepseek-v4-flash`（OpenAI 兼容 API）后，SSE 端点走 fallback 降级消息（"当前 LLM 暂不可用"），但 `ollama` 和 `gpt-4o-mini` 直接调又正常。

**根因**：`invokeStream()` 直接对 `chat.stream(messages)` 做 `for await`：
```typescript
const stream = chat.stream(messages);  // ← ChatOllama 直接返回 AsyncIterable
for await (const chunk of stream) { ... }  // ✅
```
但 `ChatOpenAI.stream()` 返回的是 **`Promise<IterableReadableStream>`**（需要 `await` 解析），直接迭代 Promise 对象抛出 `stream is not async iterable`。

**调试过程**：
```bash
# node 直接测试 ChatOpenAI.stream()
console.log('stream() constructor:', result?.constructor?.name);
// ChatOllama → "Object" (直接是 AsyncIterable)
// ChatOpenAI → "Promise" (需要 await)
```

**修复**：
```typescript
// invokeStream() 中：
const raw = await (chat as any).stream(messages);  // 先 await
const stream: AsyncIterable<unknown> =
  raw[Symbol.asyncIterator] != null
    ? raw                                          // ChatOllama 路径
    : (async function* () { yield* raw; })();      // ChatOpenAI 路径：同步 Iterable → async generator
```

**经验教训**：LangChain 不同 provider 的 `.stream()` API 行为不一致——有的直接返回 AsyncIterable，有的返回 Promise。统一处理需要 `await` + 类型检测。

---

### Bug #34 — `useSSEChat` 的 `parseSSELine` 误将 `event:` 字段识别为 `data:`

🟠 严重

**现象**：SSE 解析偶尔出现 `JSON.parse` 失败，部分 token 丢失。

**根因**：NestJS `@Sse` 装饰器发出的 SSE 格式为：
```
event: token
id: 1
data: {"content":"你","isFinal":false}
...
解析逻辑中判断顺序是：
```typescript
if (line.startsWith('data:')) return { data: ... };   // 先判断 data
if (line.startsWith('event:')) return { event: ... }; // 后判断 event
```
但实际上 `'event: token'.startsWith('data:')` 为 `false`，不会误匹配。真正的问题是：当同一事件块中有多行 `data:` 时（如 NestJS 的 MessageEvent 格式），多行被 newline 连接成无效 JSON。

**修复**：`parseSSELine` 改用 `startsWith` 精确判断，避免 substring 误匹配；空 `event` 字段时降级为 `SSEEventType.TOKEN`（NestJS 某些情况下不发 `event:` 行）。
...
**现象**：
1. AI 处理中（`content` 为空时）显示"正在思考"但光标位置不明确
2. 用户主动上滑查看历史消息时，新 token 产生会强制跳到底部，打断阅读
3. thinking 状态只有文字，无视觉反馈

**修复**：
- `MessageBubble.tsx` — thinking 状态改用弹性圆点动画（`thinking-dots` CSS animation）
- `ChatWindow.tsx` — 智能滚动：跟踪 `isNearBottomRef`，用户主动上滑（>80px）停止自动滚动，`done` 时强制滚到底
- `index.css` — 新增 `thinking-dots` 弹性球动画（3 个圆点依次弹跳）

---

### 经验教训（Phase 9 整体）

1. **流式链路要从源头开始设计**。本该在 `LlmService.invokeStream()` 设计之初就考虑异步 iterable 的跨 provider 差异，而不是在集成测试时才发现。

2. **LangChain provider 实现不一致是常态**。`.invoke()` vs `.stream()` 的返回类型在不同 provider 间可能不同（同步 vs Promise vs AsyncIterable）。封装层必须做类型检测，不能假设统一接口。

3. **智能滚动的核心是 ref 而非 state**。用 `isNearBottomRef`（ref）跟踪滚动位置而不是 re-render，靠 `useEffect` 依赖 `[messages]` 驱动——这样滚动状态不触发重渲染。
...
| Bug | 严重度 | 状态 |
|-----|--------|------|
| #32 LLM 输出同步等待 | 🔴 | ✅ `invokeStream()` + `processStream()` |
| #33 ChatOpenAI stream() 返回 Promise | 🔴 | ✅ await 解析 + Iterable 包装 |
| #34 SSE 解析误匹配 event: | 🟠 | ✅ `startsWith` 精确判断 |
| #35 前端流式 UX | 🟡 | ✅ thinking 动画 + 智能滚动 |

---

## Next Goal: Markdown 渲染

**目标**：AI 返回的分析报告（`analysis` 字段）为 Markdown 格式，前端需要渲染为富文本展示。

待办：
- 选择合适的 React Markdown 渲染库（`react-markdown` / `marked` / `@uiw/react-md-editor`）
- 支持代码高亮、表格样式、任务列表等常用 Markdown 语法
- 与现有 `AssistantCard` 样式保持一致
...
| 2026-06-23 | Phase 8 完成（Bug #27–#31 全部修复）；更新 CONFIG.md LLM API 说明 |
| 2026-06-23 | 新增 Next Goal：前端体验边界优化 |
| 2026-06-23 | Phase 9 完成（Bug #32–#35）；流式输出 + API Provider 修复；新增 Next Goal: Markdown 渲染 |
