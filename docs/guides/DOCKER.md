...
本文档介绍如何用 Docker Compose 启动完整的 AI Insight Platform 栈（PostgreSQL + NestJS 后端 + React/Vite 前端）。

LLM 调用通过云端 API（OpenAI / Anthropic），无需在本地启动推理服务。API Key 通过前端 Settings 页面配置。
...
容器名即 DNS 名（`postgres`、`server`、`web`），可互相解析。
...
# 3. 在前端 Settings 页面配置 OpenAI / Anthropic API Key
...
server 容器在 `depends_on` 中等待 `postgres healthy` 才启动。
...
### 1. LLM 调用报 401 / missing API key
进入前端 Settings 页面，配置对应 provider 的 API Key。OpenAI 默认 baseUrl 指向 `https://api.openai.com/v1`；如需走 DeepSeek 等 OpenAI 兼容端点，填入对应 baseUrl 即可。
